---
name: new-tool
description: Scaffold a new client tool on the /tools dashboard (e.g. Content Assistant, Review Responder, Competitor Scan), following the Opportunity Finder's routing, auth, sidebar, and dashboard-card pattern. Use when James is ready to build one of the "coming soon" tools into a real feature.
---

# New Tool

The `/tools` dashboard currently ships one real tool (Opportunity Finder) plus three placeholder cards for tools that don't exist yet: Content Assistant, Review Responder, Competitor Scan (defined in `COMING_SOON_TOOLS` in `src/lib/tools/reportData.ts`, rendered via `ToolPlaceholderCard`). When one of these gets built for real, it needs to be wired into several places that don't share a common registry today — this skill is the checklist.

## Touch points, in order

1. **Route:** `src/app/tools/<tool-slug>/page.tsx` — server component, auth-gated exactly like `opportunity-finder/page.tsx`:
   ```tsx
...
3. **`ToolsShell.tsx` page metadata:** add a branch to `pageMeta()` for the new route's kicker/title, e.g.:
   ...
   ```
   Order matters if the new tool also has a sub-route like `/report` — put the more specific `startsWith` check first, same as the existing `opportunity-finder/report` branch does.
...
5. **Dashboard card:** `FeaturedToolCard.tsx` is currently hardcoded to Opportunity Finder only (title, description, "4 QUICK SECTIONS" meta line are all literal strings, not props) and `src/app/tools/page.tsx` renders exactly one `<FeaturedToolCard status={toolStatus} />`. When a second real tool ships, this needs a decision, not just a copy-paste:
...
8. **Gold-standard test account (`TEST_ACCOUNT_EMAIL`, see CLAUDE.md):**
