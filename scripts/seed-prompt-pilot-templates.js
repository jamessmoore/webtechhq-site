#!/usr/bin/env node
// Seeds the prompt_pilot_template table from a gitignored local JSON file.
// Template copy is core IP and must never be committed — see
// data/seeds/prompt-pilot-templates.example.json for the shape, and copy it
// to prompt-pilot-templates.local.json (gitignored) with real content.
// Mirrors scripts/seed-prompt-templates.js for Opportunity Finder.
//
// Usage: npm run seed:prompt-pilot-templates
//   DATABASE_PATH=... to target a non-default db file
//   PROMPT_PILOT_TEMPLATE_SEED_PATH=... to target a non-default seed file
...
const SEED_PATH =
  process.env.PROMPT_PILOT_TEMPLATE_SEED_PATH ?? "./data/seeds/prompt-pilot-templates.local.json";
...
if (!fs.existsSync(resolvedSeed)) {
  ...
  console.error(
    "Copy data/seeds/prompt-pilot-templates.example.json to that path and fill in real template text.",
  );
  process.exit(1);
}
...
const replaceTemplates = db.transaction((templates) => {
  db.prepare("DELETE FROM prompt_pilot_template").run();
  ...
  for (const t of templates) {
    stmt.run(
      crypto.randomUUID(),
      t.trigger_starting_point ?? null,
      t.trigger_why ?? null,
      t.trigger_time_budget ?? null,
      t.is_fallback ? 1 : 0,
      t.version ?? 1,
      t.template_text,
      now,
...
console.log(`Seeded ${seed.templates.length} Prompt Pilot templates into ${resolvedDb}`);
